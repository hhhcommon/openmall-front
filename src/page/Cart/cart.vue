                  ...
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <div class="items-thumb fl">
                        ...
                        <a @click="goodsDetails(item.id)" :title="item.productName" target="_blank"></a>
                      </div>
                      <!--信息-->
...
export default {
  ...
  methods: {
    ...mapMutations(["INIT_BUYCART", "EDIT_CART", "REDUCE_CART","DEL_CART"]),
    message(m) {
      this.$message.error({
        message: m
      });
    ...
    // 全选
    editCheckAll() {
      this.EDIT_CART({ checked: this.checkAllFlag ? "0" : "1" });
    },
    // 修改购物车
    ...
    EditNum(quantity, id) {
      ...
      // 数量
      this.EDIT_CART({ productId: id, quantity: quantity });
      cartQuantity({
        id,
        quantity
      }).then(res => {
