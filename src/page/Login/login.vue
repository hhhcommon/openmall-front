        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            ...
            <li style="text-align: right" class="pr">
              ...
              <a style="padding: 1px 0 0 10px" @click="open('找回密码','test | test')">忘记密码 ?</a>
            </li>
          </ul>
          <!--登陆-->
  ...
  methods: {
    ...
    loginSubmit() {
      ...
      userLogin(params).then(res => {
        console.log(res);
        if (res.code === 200) {
          setStore("Authorization", res.data.tokenHead + " " + res.data.token);
          // 登录后添加当前缓存中的购物车
            ...
            this.cartList.forEach(item => {
              cartAdd({
                ...
                if (n == this.cartList.length) {
                  removeStore("buyCart");
                  let lastPage = getStore("LAST_PAGE");
                  if (lastPage !== "/login") {
...
.v2 .dialog {
  ...
  .title {
    ...
    position: relative;
    background-image: url(/static/images/u54.png);
